...
/// 指定子を commit sha に解決する。手元で完結する場合（完全な sha が既に
/// mirror にある）を除き、上流の最新の参照を取り込んでから解決する。
pub fn resolve(home: &Home, url: &str, spec: &Spec) -> Result<String, String> {
    ...
    let sha = match spec {
        ...
        Spec::Version(v) => [format!("v{v}"), v.clone()]
            .iter()
            .find_map(|t| rev_parse(&mirror, &format!("refs/tags/{t}^{{commit}}")))
        ...
        Spec::Tag(t) => rev_parse(&mirror, &format!("refs/tags/{t}^{{commit}}"))
...
/// 解決してビルドし、`versions/<sha>/` に置く。既に在れば何もしない。
