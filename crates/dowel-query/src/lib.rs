...
//! 依存が導出クエリの場合、それを最新にするにはその計算手続きが必要になる。
//! 呼び出し側から渡るのは自身の手続きのみであるため、メモが手続きを保持していない場合、
//! 依存が古いという理由だけで自身を再計算することになる。この場合、導出が
//! 連なった経路で early cutoff が機能しない。
//!
//! この制約は記述側にも及ぶ。計算手続きは `Db` からのみ値を読める（`'static`）。
//! 外部の状態を捕捉した手続きは保存できず、純粋性が型によって保証される。
//!
//! ## キャンセルを `Result` にした理由
//!
...
//! ## 何を指紋にするか
//!
//! 指紋は値の内容を表す必要がある。依存側が観測できる要素が指紋に含まれていない場合、
//! early cutoff が変化なしと判定した後も、依存側は古い派生結果を保持する。
//! 値がスパンを含む場合、指紋にもスパンを含める。
