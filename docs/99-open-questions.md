...
**状態**: 前提は揃った。格納する対象の選択が未決。

`FileId` の扱いは [ADR-0009](adr/0009-file-identity.md)、直列化の形式は
`dowel_eval::codec`、派生のクエリ化と cutoff の条件は
[ADR-0011](adr/0011-cutoff-and-provenance.md) で決定済みである。

残るのは、プロセスを跨いで何を格納するかである。

| 対象 | 格納の利得 | 復元の代償 |
|---|---|---|
| `Evaluated`（ファイル単位） | 解析と評価を省く | 値が大きい。スパンを含むため本文が変われば必ず無効 |
| `Interface` / `CompileEnv`（ターゲット単位） | 併合を省く。要約が同じなら本文が変わっても有効 | 依存のインタフェースを辿るため、単独では復元できない |

後者だけを格納しても、`Declared` と `Deps` は毎回の読み込みで作り直す必要がある。
`Declared` を作るには評価が要るため、前者を省くには両方を格納することになる。

未決の点:

- 両方を格納するか、`Evaluated` だけにするか
- 値ログに書く単位（1クエリ1レコードか、ファイル単位でまとめるか）

## Q10. dowelup の prebuilt 配布

**状態**: 未着手。[ADR-0012](adr/0012-self-acquisition.md) はソースビルドのみを定めた。
