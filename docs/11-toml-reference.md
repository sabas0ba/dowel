...
| `asm` | string | the assembler command. **Empty by default**, which means the C driver assembles — that is what `.s` and `.S` need ([ADR-0048](adr/0048-assembly.md)) and what nearly every tree wants. Declaring one sends **every** assembly source in the build to it, including `.s` and `.S`, and dowel then passes only the input, the output, and `asm_flags` ([ADR-0050](adr/0050-separate-assembler.md)). A `.asm` source needs one, since that spelling is MASM or NASM syntax: without a declaration it is `missing-assembler`. Missing from PATH: `missing-toolchain` |
